...
1. **AI時代のエンジニアリング** - 私たちの仕事はどう変わるのか
2. **ソシオテクニカルアーキテクチャ** - ソフトウェアと組織を同時に設計する
3. **イベントストーミング** - ビジネスを可視化する協働的手法
4. **DDD × Team Topologies** - 統合的な活用
5. **レガシーからの脱却** - 競争優位性への転換
6. **まとめ** - 意志を実装するということ
...
- レガシーシステムの改善が進まない
- 技術的な改善と組織の問題が絡み合っている
- AIでコードは書けるが、設計の判断ができない
...
- アーキテクチャモダナイゼーションの全体像
- ソシオテクニカルな視点での設計アプローチ
- 協働的な設計手法（イベントストーミング等）
...
- AI時代、「何を作るか」「なぜ作るか」を決める力が重要になる
- 技術が楽になると、人間の問題がボトルネックとして露わになる
- 技術的負債は複利で増え、しかし銀の弾丸はない
...
<div style="margin-top: 15px; padding: 12px; background-color: #e0e0e0; border-radius: 5px; text-align: center;">
<span style="color: #e65100; font-weight: bold;">Bounded Context は「分断」ではなく「明確化」。境界を認めることで、かえって連携しやすくなる。</span>
</div>
...
|---------|------|
| **パートナーシップ** | 両チームが対等に協力 |
| **カスタマー・サプライヤー** | 上流が下流に対応 |
| **順応者** | 下流が上流に従う |
| **腐敗防止層（ACL）** | 外部から保護 |

Team Topologiesのインタラクションモードと対応する。
...
# 4. DDD × Team Topologies

</div>

<div style="font-size: 0.9em; margin-top: 30px; color: #aaa;">

ドメイン駆動設計とチーム設計の</br>統合的な活用
...
- **Collaborationは持続不可能**: 2チームが常に密に連携すると、両方の速度が落ちる
- **X-as-a-Serviceは最初から無理**: 境界が不明確な段階でAPIを固定すると、後で苦しむ
- **Facilitatingは橋渡し**: 知識を移転し、自律性を高める
...
- **認知負荷の最小化**: チームは自分のドメインに集中できる
- **依存関係の明確化**: コンテキストマップがチーム間関係を示す
- **変更の局所化**: ドメイン内の変更は同じチーム内で完結
- **スケーラビリティ**: ビジネスの成長に合わせてチームを増やせる
...
- 技術が楽になると、人間の問題がボトルネックになる
- ソシオテクニカル: ソフトウェアと組織を同時に設計
- イベントストーミングで協働的にドメインを発見
- DDDとTeam Topologiesの統合で境界を一致させる
...
- **どんな未来を描くか**を決めること
- その実現に向けて**技術と組織の両面**から設計すること
- **チームと協働**して継続的に進化させること
